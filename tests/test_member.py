from crypto import DispersyPrivateKey
...
class TestMember(DispersyTestFunc):
    ...
    def _test_verify(self, security_level):
        ...
        self.assertTrue(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "0123456789"), offset=0, length=0))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "0123456789"), offset=0, length=0))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "0123456789"), offset=0, length=9))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "0123456789"), offset=0, length=9))
        self.assertTrue(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "0123456789"), offset=0, length=10))
        self.assertTrue(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "0123456789"), offset=0, length=10))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "0123456789"), offset=0, length=11))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "0123456789"), offset=0, length=11))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "0123456789"), offset=0, length=666))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "0123456789"), offset=0, length=666))

        # sign and verify "0123456789"[1:10]
        self.assertTrue(member.verify("123456789", self._dispersy.crypto.create_signature(key, "123456789")))
        self.assertTrue(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "123456789"), offset=1, length=0))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "123456789"), offset=1, length=0))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "123456789"), offset=1, length=8))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "123456789"), offset=1, length=8))
        self.assertTrue(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "123456789"), offset=1, length=9))
        self.assertTrue(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "123456789"), offset=1, length=9))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "123456789"), offset=1, length=10))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "123456789"), offset=1, length=10))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "123456789"), offset=1, length=666))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "123456789"), offset=1, length=666))

        # sign and verify "0123456789"[0:9]
        self.assertTrue(member.verify("012345678", self._dispersy.crypto.create_signature(key, "012345678")))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "012345678"), offset=0, length=0))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "012345678"), offset=0, length=0))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "012345678"), offset=0, length=8))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "012345678"), offset=0, length=8))
        self.assertTrue(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "012345678"), offset=0, length=9))
        self.assertTrue(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "012345678"), offset=0, length=9))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "012345678"), offset=0, length=10))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "012345678"), offset=0, length=10))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "012345678"), offset=0, length=666))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "012345678"), offset=0, length=666))

        # sign and verify "0123456789"[1:9]
        self.assertTrue(member.verify("12345678", self._dispersy.crypto.create_signature(key, "12345678")))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "12345678"), offset=1, length=0))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "12345678"), offset=1, length=0))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "12345678"), offset=1, length=7))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "12345678"), offset=1, length=7))
        self.assertTrue(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "12345678"), offset=1, length=8))
        self.assertTrue(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "12345678"), offset=1, length=8))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "12345678"), offset=1, length=9))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "12345678"), offset=1, length=9))
        self.assertFalse(member.verify("0123456789", self._dispersy.crypto.create_signature(key, "12345678"), offset=1, length=666))
        self.assertFalse(member.verify("0123456789E", self._dispersy.crypto.create_signature(key, "12345678"), offset=1, length=666))
